  ...
  - **Conformance state vs protobuf v25.9**: was `1071 successes,
    ...
    **Codec correctness pass:**
      * UInt32 vbyte tolerates the 6-/9-/10-byte sign-extended forms
        ...
        RepeatedScalarSelectsLast.UINT32 + ValidDataRepeated.UINT32
        Packed/Unpacked failures (each in proto2 + proto3, with
        Json/Protobuf output mirrors).
      * BOOL packed decode moved out of the fixed-size dispatch
        (varints, not `sizeof(Bool)==1` bytes). Fixes
        ValidDataRepeated.BOOL.PackedInput.
      * `decode(::Vector{UInt8})` / `decode(::Base.CodeUnits)` now
        check `bytesavailable` before the read so truncated
        length-delimited known-field payloads error instead of
        silently returning short. Removes
        PrematureEofInDelimitedDataForKnown{Non,}RepeatedValue.BYTES.
      * proto2 `optional` ENUM fields now go through the same
        `_wants_scalar_presence` predicate as scalars — typed
        `Union{Nothing,EnumT}`, skip on `nothing` on encode. Fixes
        ValidDataScalar.ENUM[0] (the "set explicitly to first enum
        value" round-trip).
      * `_merge_structs` rewritten to inline-handle every codegen
        field shape: presence-bearing scalars / submessages
        (`Union{Nothing,T}`), repeated (vcat), maps (Base.merge),
        bare submessages, and a proto3 default-skip heuristic for
        bare scalars (`s2.field === default ? s1.field : s2.field`)
        so two-message merging matches wire-level semantics without
        per-field presence tracking. Adds OneOf-vs-OneOf in the
        parent module. Fixes RepeatedScalarMessageMerge.
      * Codegen oneof MESSAGE-member decoder seeds the temporary
...
After Phase 8 the bootstrap is generated by our own codegen — proto2
`required` is validated and proto2 `optional` scalar presence is correct.
One residual:

- The committed bootstrap (`gen/google/`) was produced before the proto2
  ...
  bootstrap-regen pass would close the gap and matches the user-facing
  codegen behavior. Doing the regen requires care: codegen.jl itself
  reads `field.label` / `field.type` (now potentially `Union{Nothing,…}`)
  via `===` comparisons that already handle the Nothing case correctly,
  but a regen would still want a CI check that no diff sneaks in
  unintentionally.
