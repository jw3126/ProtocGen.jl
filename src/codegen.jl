...
using ..ProtocGenJulia: ProtocGenJulia
using ..ProtocGenJulia.google.protobuf: FieldDescriptorProto, DescriptorProto,
...
const WKT_PACKAGE_MAP = Dict{String,String}(
    "google.protobuf" => "ProtocGenJulia.google.protobuf",
)
...
function codegen(file::FileDescriptorProto, universe::Universe)
    ...
    syntax = something(file.syntax, "proto2")
    println(io, "# Generated by ProtocGenJulia. Do not edit.")
    println(io, "# source: ", proto_name, " (", syntax, " syntax)")
    println(io, "#! format: off")
    println(io)
    println(io, "import ProtocGenJulia as PB")
    println(io, "using ProtocGenJulia: OneOf, OrderedDict")
    # Pull the user-facing codec entry points into the include site so
    # `encode(io, msg)` / `decode(io, T)` / `encode_json` / `decode_json`
    # work without the caller importing ProtocGenJulia themselves.
    println(io, "using ProtocGenJulia: encode, decode, encode_json, decode_json")
    println(io, "using ProtocGenJulia.EnumX: @enumx")
...
function codegen_driver(file_to_generate::Vector{String},
    ...
    # without needing their own `using ProtocGenJulia`.
    println(io, "using ProtocGenJulia: encode, decode, encode_json, decode_json")
    println(io)
