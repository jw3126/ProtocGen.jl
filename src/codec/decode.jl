...
function _decode!(d::AbstractProtoDecoder, buffer::AbstractDict{K,V}) where {K,V}
    io = get_stream(d)
    pair_len = vbyte_decode(io, UInt32)
    pair_end_pos = position(io) + pair_len
    key = _map_default(K)
...
function _decode(d::AbstractProtoDecoder, ::Type{String})
    bytelen = vbyte_decode(get_stream(d), UInt32)
    str = Base._string_n(bytelen)
    Base.unsafe_read(get_stream(d), pointer(str), bytelen)
    return str
end
function _decode!(d::AbstractProtoDecoder, buffer::BufferedVector{String})
    buffer[] = _decode(d, String)
    return nothing
end

function _decode(d::AbstractProtoDecoder, ::Type{Vector{UInt8}})
    io = get_stream(d)
...
function _decode!(d::AbstractProtoDecoder, buffer::BufferedVector{Vector{UInt8}})
    buffer[] = _decode(d, Vector{UInt8})
    return nothing
end
...
# This method handles messages decoded as OneOf / repeated. We expect `_decode(d, T)`
...
# We don't reuse the _decode!(d::AbstractProtoDecoder, buffer::Base.RefValue{T}) method above
# as with OneOf fields, we can't be sure that the previous OneOf value was also T.
function _decode(d::AbstractProtoDecoder, ::Type{Ref{T}}) where {T}
    io = get_stream(d)
    bytelen = vbyte_decode(io, UInt32)
    endpos = bytelen + position(io)
    if hasmethod(_decode, Tuple{AbstractProtoDecoder,Type{T},Int,Bool})
        ...
        out = _decode(LengthDelimitedProtoDecoder(get_stream(d), endpos), T)
    end
    @assert position(io) == endpos "$(T) decode: expected position $(endpos), got $(position(io))"
    return out
end

function _decode!(d::AbstractProtoDecoder, buffer::BufferedVector{T}) where {T}
    buffer[] = _decode(d, Ref{T})
    return nothing
end
...
function _decode(d::AbstractProtoDecoder, ::Type{Ref{T}}, ::Type{Val{:group}}) where {T}
    if hasmethod(_decode, Tuple{AbstractProtoDecoder,Type{T},Int,Bool})
        ...
        _warn_old_decode_method()
        out = _decode(GroupProtoDecoder(get_stream(d)), T)
    end
    return out
end
...
function _decode!(d::AbstractProtoDecoder, buffer::Base.RefValue{S}) where {S>:Nothing}
    T = Core.Compiler.typesubtract(S, Nothing, 2)
    if !isnothing(buffer[])
        buffer[] = _merge_structs(getindex(buffer)::T, _decode(d, Ref{T}))
    else
        buffer[] = _decode(d, Ref{T})
    end
    return nothing
...
function _decode!(d::AbstractProtoDecoder, buffer::Base.RefValue{T}) where {T}
    if isassigned(buffer)
        buffer[] = _merge_structs(buffer[], _decode(d, Ref{T}))
    else
        buffer[] = _decode(d, Ref{T})
    end
    return nothing
