...
# Phase 7: all 11 well-known types under
# `ProtocGenJulia.google.protobuf.*`, generated by our own codegen
# via `gen/regen_wkt.jl`.
...
const WKT = ProtocGenJulia.google.protobuf

function rt(x::T) where {T}
    bytes = Base.invokelatest(ProtocGenJulia.encode, x)
    decoded = Base.invokelatest(ProtocGenJulia.decode, bytes, T)
